$(function() {
  ...
  function addParticipantsMessage (data) {
    if(data.hasOwnProperty('loggedInUser')){
      $user.html('Hi '+data.loggedInUser);
    }
    $users.html('');
    for(var key in data.users){
        $users.append($('<li><i style="color:green" class="fa fa-circle" aria-hidden="true"></i>'+data.users[key]+'</li>'));
    }
    $('.active').val('Active('+data.users.length+')');
    var message = '';
    if (data.numUsers === 1) {
  ...
  // Whenever the server emits 'previous message', update the chat body
  socket.on('previous messages', function (data) {
    var prevM = data.data;
    for(var key in prevM){
      addChatMessage(prevM[key]);
    }
  });
